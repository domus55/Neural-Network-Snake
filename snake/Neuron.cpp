...
void Neuron::update(float wygrana)
{
	...
	if (rozegraneGry % iloscGier == 0)
	{
		...
		{
			neuron[i].Mutacja((1 - procent) * 5.0);
		}
	}
	...
	if (rozegraneGry + 1 == iloscGier * iloscPotomkow)
	{
		...
		do
		{
			...
			for (int i = 0; i < iloscPotomkow; i++)
			{
				...
				if (znalezionychBotow >= iloscNajlepszychPotomkow * 2) break;
			}
...
void Neuron::Mutacja(float mutujMocno)
{
	...
	if (!outputLayer)
	{
		...
		{
			int i = rand() % iloscWag;
			float mutacja = ((rand() % 2001) - 1000) / 2000.0;
			waga[i] = mutacja;
		}
	}

	//float mutacja = ((rand() % 3) - 1) / 10000.0;
	//bias += mutacja;
}
